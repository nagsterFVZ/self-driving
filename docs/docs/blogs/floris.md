...
## Christmas Holidays: 26-12-2022 - 08-01-2023

During the holidays I spent time on trying to get AirSim functional with reinforcement learning. I managed to get to the point where Python code could drive the vehicle. I also tried to implement multiple ways of reinforcement learning to no success. Projects that I had found using RL for AirSim were either out of date or for drones and not vehicles.

## Week 16: 09-01-2023

During this week I started to do some line detection with OpenCV. The line detection works to some extent but will need more time to fine tune and to create an alogrithm to make the vehicle drive accordingly. I also made some changes to the vehicle control system, we did our first proper test drive with manual controls and managed to drive the car into a wall. From this we learned some key things: we needed better manual controls, testing should be done in larger open spaces and to program in safety features for controller disconnect.

## Week 17: 16-01-2023

Over the weekend I revamped the control system for manual controls. We can now connect to the vehicle's wifi with a computer or tablet, then use an Xbox controller to arm, calibrate and drive the vehicle. This control scheme works a lot better but does take some getting used to. We drove arround the entire Nexus building and we found that: the car can easily pull weight, is very responsive, uses minimal battery power and we needed to add some locktight to some bolts. The rest of the week was spent on finalizing research reports and the final project report.
...
During this weeks feedback session we just went over our personal development plans, and discussed what sort of questions we can expect for the final evaluation.
